...
|    |            | 5.10(2 point) I think problem should be If $L_{FN} = cL_{FP}$, show that we should pick $\hat{y}$ = 1 if $p(y =1 \text{ given } x) > \tau$ where $\tau = (c)/(1+c)$ *Let me know if someone thinks book version is right.*           |                                  |     |
