...
| - Use equation 4.43( $\boldsymbol{x}^T \mathbf{\beta_c - \beta_{c^{'}} } =\gamma_c -\gamma_{c^{'}})$) from the book and show that decision boundary is perpendicular to the line connecting two mean $\mu_c$ and $\mu_{c^'}$ |    |    |                                          |     |   |
